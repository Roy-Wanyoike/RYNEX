...
// Load .env relative to the compiled file location (dist/config -> Background-Services/.env)
...
// Cached connect promise (node-mssql caches the pool per config; this guards
// against our own double-connects).
let connectionPromise: Promise<void> | null = null

/**
 * Connects once and caches the result. Idempotent and safe to call
...
export async function ensureConnection(): Promise<void> {
  if (connectionPromise === null) {
    connectionPromise = mssql.connect(sqlConfig).then(
      () => undefined,
      (error: unknown) => {
